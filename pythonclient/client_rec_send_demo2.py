...
async def hello():
    ...
    async with websockets.connect(uri) as websocket:
        ...
        while True:
            ...
            # add face detect module
            img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            img_gray = cv2.resize(img_gray, None, fx=0.5, fy=0.5)
            rect_round = detector(img_gray)
            for i, d in enumerate(rect_round):
                img = cv2.rectangle(img, (d.left()*2, d.top()*2),
                                    (d.right()*2, d.bottom()*2), (255, 0, 0), 3)
            #
            pic_data = cv2.imencode('.jpeg', img)[1].tobytes()

            # pic_data = np.tobuffer(pic_data)
